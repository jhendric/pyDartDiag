...
for dday in datevec_simu_month:
   ...
   GS_paths_T=[]
   GS_paths_Z=[]
   ncfile = dir_obs+'GS_paths_hind_'+dday.strftime('%Y%m')+'.nc'
   if not os.path.isfile(ncfile):
      for i_simu in xrange(N_hindc):
         print " - hindcast " + simus_hindc[i_simu]
         # Get data
         file_avg = dir_hindc[i_simu] + simus_hindc[i_simu] + '/monthly/' + simus_hindc[i_simu] + '_avg_' + dday.strftime('%Y-%m') + '_monthly.nc'
...
#%% Reinterpolation
#########################################################################################
      GS_paths_mat = np.empty((2,N_hindc,N_interp,2))
...
#########################################################################################
      ...
      nc_file.add_variable('GS_paths'        ,np.expand_dims(GS_paths_mat,axis=0),\
                vardim=('time','variable','simu','along_coord','coordinates'),varatt=path_att)
